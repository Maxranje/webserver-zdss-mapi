...
class ClassLoader
{
    ...
    {
        // PSR-4 lookup
        // $logicalPathPsr4 = strtr($class, '_', DIRECTORY_SEPARATOR) . $ext;
        // $first = $class[0];
        ...
        // PSR-0 lookup
        $segment_class = explode('_', $class);
        if (!$segment_class || !is_array($segment_class) || count($segment_class) < 2) {
            return false;
        }
        ...
        if (isset($this->prefixesPsr0[$segment_class[0]])) {
            ...
            $isRealFromUc = false;
            if (strpos($logicalPathPsr0, "twig") !== false) {
                $isRealFromUc = true;
            }
            for ( $i = 1; $i < count($segment_class) - 1; $i++ ) {
                if ($isRealFromUc) {
                    $logicalPathPsr0 .= $segment_class[$i] . DIRECTORY_SEPARATOR;
                } else {
