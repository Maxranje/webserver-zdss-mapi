...
class Service_Page_Mock_Question_Create extends Zy_Core_Service{
    ...
    public function execute () {
        ...
        $radio      = empty($this->request['single_answers_radio']) ? array() : $this->request['single_answers_radio'];
        $checkbox   = empty($this->request['single_answers_checkbox']) ? array() : $this->request['single_answers_checkbox'];
        $check      = empty($this->request['single_answers_check']) ? 0 : intval($this->request['single_answers_check']);
        $fill       = empty($this->request['single_answers_fill']) ? array() : $this->request['single_answers_fill'];
        $write      = empty($this->request['single_answer_write']) ? "" : trim($this->request['single_answer_write']);
