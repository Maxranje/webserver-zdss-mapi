...
// 排课列表
class Service_Page_Schedule_Band_Apcreate extends Zy_Core_Service{
    ...
    public function execute () {
        ...
        if (empty($orderData)) {
            throw new Zy_Core_Exception(405, "操作失败, 无法获取服务订单信息");
        }
        $orderData["ext"] = json_decode($orderData['ext'], true);
        if (empty($orderData['ext']["schedule_nums"])) {
            throw new Zy_Core_Exception(405, "操作失败, 获取的服务订单信息不正确");
        }
        ...
        if (empty($apackageInfo)) {
            throw new Zy_Core_Exception(405, "操作失败, 无法获取服务信息");
        }
        if (!in_array($apackageInfo['state'], Service_Data_Aporderpackage::APORDER_STATUS_ABLE_MAP)) {
            throw new Zy_Core_Exception(405, "操作失败, 订单归属服务不是有效服务, 无法操作, 请检查服务");
        }

        // check 学生信息
        $serviceUser = new Service_Data_Profile();
        $userInfo = $serviceUser->getUserInfoByUid($orderData['student_uid']);
        if (empty($userInfo) || $userInfo['state'] == Service_Data_Profile::STUDENT_DISABLE) {
            throw new Zy_Core_Exception(405, "操作失败, 学员信息获取失败");
        }
        if (!$this->isOperator(Service_Data_Roles::ROLE_MODE_STUDENT_AMOUNT_HANDLE, $userInfo['sop_uid'])) {
        ...
        $subjectInfo = $serviceData->getSubjectByParentID(intval($orderData['subject_id']));
        ...
        // 已绑定 & 新增绑定  & 删除的绑定
        $bandDuration = $newBandDurtaion = $delBandDuration = 0;  

        foreach ($curriculum as $item) {
            $bandDuration += $item['end_time'] - $item['start_time'];
        ...
        if (!empty($newScheduleIds)) {
            ...
            foreach ($newScheduleIds as $id) {
                ...
                $newBandDurtaion += $newSchedules[$id]['end_time'] - $newSchedules[$id]['start_time'];
            }
            ...
            $ret = $serviceCurriculum->checkStudentTimes($needTimes1, $needDays, intval($orderData['student_uid']));
            if ($ret === false) {
        ...
        if (!empty($delScheduleIds)) {
            ...
            foreach ($delScheduleIds as $id) {
                ...
                $delBandDuration -= $schedules[$id]['end_time'] - $schedules[$id]['start_time'];
            }
        }
        
        // 判断选定课程是否够排
        $bandDuration = $bandDuration / 3600; 
        $newBandDurtaion = $newBandDurtaion / 3600;
        $delBandDuration = $delBandDuration / 3600;

        if ($bandDuration + $newBandDurtaion + $delBandDuration > $orderData['ext']['schedule_nums']) {
            throw new Zy_Core_Exception(405, 
                sprintf("操作失败, 订单配置排课数不足, 订单配置课时:%s小时, 已排课:%s小时, 新增排课:%s小时, 删减排课:%s小时",
                $orderData['ext']['schedule_nums'],
                $bandDuration, 
                $newBandDurtaion,
                $delBandDuration
            ));
