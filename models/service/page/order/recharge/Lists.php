...
class Service_Page_Order_Recharge_Lists extends Zy_Core_Service{
    ...
    public function execute () {
        ...
        if ($isExport) {
            ...
            Zy_Helper_Utils::exportExcelSimple("order_balance", $data['title'], $data['lists']);
        }
    ...
    private function formatDefault ($lists) {
        ...
        foreach ($lists as $v) {
            ...
            $v['schedule_nums']     = sprintf("%.2f", floatval($v['schedule_nums']));
    ...
    private function formatExcel($lists) {
        $result = array(
            'title' => array('订单ID', 'UID', '学员名', '类型', '金额(元)', '课时(小时)', '操作员',  '更新时间'),
            'lists' => array(),
        );
        ...
        foreach ($lists as $item) {
            $tmp = array(
                ...
                $item['student_uid'],
                $item['student_name'],
                $item['type'] == Service_Data_Recharge::RECHARGE_ORDERCREATE? "创建" : "结转",
                $item['balance'],
                $item['schedule_nums'],
                $item['operator'],
                $item['update_time'],
